...
class Solution(object):
    def predictPartyVictory(self, senate):
        ...
        while 1:
            if senates[index] == "R":
                j = 1
                while (senates[(index + j) % lens] != 'D' and j < lens):
                    j+=1
                if (senates[(index + j) % lens] == 'R'):
                    return "Radiant"
                else:
                    senates[(index + j) % lens] = 'V'

                # announce the victory
                # if "D" not in senates: return "Radiant"

            elif senates[index] == "D":
                j = 1
                while (senates[(index + j) % lens] != 'R' and j < lens):
                    j+=1
                if (senates[(index + j) % lens] == 'D'):
                    return "Dire"
                else:
                    senates[(index + j) % lens] = 'V'

                # announce the victory
                # if "R" not in senates: return "Dire"

            # index = index + 1 if index < len(senates) - 1 else 0
            index = (index + 1) % len(senates)
        """
